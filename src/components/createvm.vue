  ...
  <div style="background-color: black">
    ...
    <div class="col-sm-12" style="margin-top:20px;background-color: #f9e7ba; padding-top: 20px; box-shadow: 0 0.75rem 1.5rem rgba(18, 38, 63, 0.03)">
      <div class="col-sm-12 form-group">
        ...
        <div class="col-sm-9">
          <label :class="datacenterclick == index ? 'addclass' : 'labelbackcolordefault'" v-for="(c, index) in datacenter" :key="c.id" @click="clickdatacenter(index)">{{ c.Datacenter }}</label>
        </div>
      </div>
      <div class="col-sm-12 form-group">
        <div class="col-sm-3">
        ...
        <div class="col-sm-9">
          <label :class="storageclick == index ? 'addclass' : 'labelbackcolordefault'" v-for="(c, index) in storage" :key="c.id" @click="clickstorage(index)">{{ c.Name }}</label>
        </div>
      </div>
      ...
      <div class="col-sm-12 form-group">
        ...
        <div class="col-sm-3">
          <select style="margin-left: 5px" class="col-sm-12" v-model="vlanvalue" @change="gethosts(centervalue)">
            <option value="">--请选择--</option>
            <option v-for="v in vlanlist" :key="v.id" :value="v.Vlan">
              {{ v.Vlan }}
            </option>
          </select>
        </div>
        <div class="col-sm-4">
          <select class="col-sm-12" v-model="ipvalue" style="margin-left: 10px">
            <option value="">--请选择--</option>
            <option v-for="ip in iplist" :key="ip.id" :value="ip.Ipv4">
              {{ ip.Ipv4 }}
            </option>
          </select>
        </div>
      ...
      <div class="col-sm-12 form-group">
        ...
        <div class="col-sm-9">
          <label :class="cpuclick == index ? 'addclass' : 'labelbackcolordefault'" v-for="(c, index) in cpulist" :key="c.id" @click="clickcpu(index)">{{ c }}核心</label>
        </div>
      </div>
      <div class="col-sm-12 form-group">
        <div class="col-sm-3">
        ...
        <div class="col-sm-9">
          <label :class="memclick == index ? 'addclass' : 'labelbackcolordefault'" v-for="(c, index) in memlist" :key="c.id" @click="clickmem(index)">{{ c }}G</label>
        </div>
      </div>
      <div class="col-sm-12 form-group">
        <div class="col-sm-3">
        ...
        <div class="col-sm-3">
          <select style="margin-left: 5px" class="col-sm-12" v-model="tagvalue" @change="getimagebytag">
            <option value="">--请选择--</option>
            <option v-for="o in ostaglist" :key="o.Id" :value="o.Id">
              {{ o.Tag }}
            </option>
          </select>
        </div>
        <div class="col-sm-4">
          <select style="margin-left: 5px" class="col-sm-12" v-model="imagevalue">
            <option value="">--请选择--</option>
            <option v-for="image in imagelist" :key="image.Id" :value="image.Id">
              {{ image.Osname }}
            </option>
          </select>
        </div>
      ...
      <div class="col-sm-12 form-group">
        ...
        <div class="col-sm-3 title">
          <select class="col-sm-12" v-model="hostvalue" style="margin-left: 5px">
            <option value="">--请选择--</option>
            <option v-for="host in hostlist" :key="host.Ipv4" :value="host.Ipv4">
              {{ host.Ipv4 }} ({{ host.Usedcpu }}/{{ host.Cpu }}核，{{ host.Usedmem }}/{{ host.Mem }}G ，{{ host.count }}/{{ host.Max_vms }}个)
            </option>
          </select>
        </div>
      </div>
      ...
      <div class="col-sm-12 form-group create">
        ...
        <div class="col-sm-2">
          <button class="btn btn-success btn-sm" style="margin-left: 5px" @click="createvm()">
            创建
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    gethosts: function(datacenter) {
      ...
      this.$http
        .get(apiurl, {
          params: { datacenter: datacenter, vlan: this.vlanvalue }
        })
        .then(response => {
          if (response.data.err === null) {
