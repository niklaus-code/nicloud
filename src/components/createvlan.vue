...
export default {
  ...
  methods: {
    check: function(datacenter, vlan, bridge, network, prefix, gateway) {
      if (
        typeof datacenter === 'undefined' ||
        datacenter === null ||
        datacenter === '' ||
        typeof vlan === 'undefined' ||
        vlan === null ||
        vlan === '' ||
    ...
    createvlan: function() {
      ...
      this.$http
        .post(apiurl, this.$qs.stringify({ datacenter: this.centervalue, vlan: this.vlan, bridge: this.bridge, network: this.network, prefix: this.prefix, gateway: this.gateway }))
        .then(response => {
          if (response.data.err === null) {
            alert('创建成功! 是否查看网络列表')
