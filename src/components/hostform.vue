          ...
          <div class="col-sm-8" style="float: left; max-width: 360px">
            <div style="width: 45%; float: left ">
              <div class="panel panel-success">
                ...
                <ul class="panel-body">
                  <li v-for="(item, index) in vlanlist" :key="item.id" @click="movevlan(index)">{{ item }}</li>
                </ul>
              </div>
            </div>
            ...
            <div style="width: 45%;float: left ">
              <div class="panel panel-success">
                ...
                <ul class="panel-body">
                  <li v-for="(item, index) in choosen_vlan" :key="item.id" @click="backvlan(index)">{{ item }}</li>
                </ul>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    commit: function() {
      ...
      this.$http
        .post(apiurl, this.$qs.stringify({ datacenter: this.centervalue, cpu: this.cpu, mem: this.mem, ipv4: this.ip, max_vms: this.num, vlan: JSON.stringify(this.choosen_vlan) }))
        .then(response => {
          if (response.data.err === null) {
