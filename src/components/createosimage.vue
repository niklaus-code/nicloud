    ...
    <div class="col-sm-12" style="background: rgb(249, 231, 186); padding: 10px">
      <div class="col-sm-12">
        <div class="form-group">
          ...
          <div class="col-sm-6">
            <label :class="datacenterclick == index ? 'addclass' : 'labelbackcolordefault'" v-for="(x, index) in datacenter" :key="x.id" @click="clickdatacenter(index)">{{ x.Datacenter }}</label>
          </div>
        </div>
      </div>
      <div class="col-sm-12" style="margin-top:10px">
        <div class="form-group">
          ...
          <div class="col-sm-6">
            <label :class="storageclick == index ? 'addclass' : 'labelbackcolordefault'" v-for="(x, index) in storage" :key="x.id" @click="clickstorage(index)">{{ x.Name }}</label>
          </div>
        </div>
      </div>
      <div class="col-sm-12" style="margin-top:10px">
        <div class="form-group">
          ...
          <div class="col-sm-6">
            <label :class="sortclick == index ? 'addclass' : 'labelbackcolordefault'" v-for="(x, index) in sort" :key="x.id" @click="clicksort(index)">{{ x.Sort }}</label>
          </div>
        </div>
      </div>
      <div class="col-sm-12" style="margin-top:10px">
        <div class="form-group">
          ...
          <div class="col-sm-6">
            <label :class="tagclick == index ? 'addclass' : 'labelbackcolordefault'" v-for="(x, index) in ostags" :key="x.id" @click="clicktag(index)">{{ x.Tag }}</label>
          </div>
        </div>
      </div>
      <div class="col-sm-12" style="margin-top:10px">
        ...
        <div class="form-group">
          ...
          <div class="col-sm-6">
            <div class="checkbox" style="margin-top:0">
              <label style="float: left"> <input type="checkbox" @click="checkbox" style="margin-top: -6px" /><span style="color: #999">勾选创建快照并以此为基础克隆镜像</span> </label>
            </div>
          </div>
        </div>
      </div>
      <div class="col-sm-12" style="margin-top: 10px">
        <div class="form-group">
          ...
          <div class="col-sm-6">
            <label :class="xmlclick == index ? 'addclass' : 'labelbackcolordefault'" v-for="(x, index) in xmllist" :key="x.id" @click="clickxml(index)">{{ x.Comment }}</label>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    check: function(osname, cephblockdevice, xml) {
      if (
        typeof osname === 'undefined' ||
        osname === null ||
        osname === '' ||
        typeof cephblockdevice === 'undefined' ||
        cephblockdevice === null ||
        cephblockdevice === '' ||
      ...
      this.$http
        .post(
          ...
          this.$qs.stringify({
            ...
            cephblockdevice: this.cephblockdevice,
            createsnap: this.checkboxobj,
            xml: this.xmlvalue,
            ossort: this.sortvalue.Id,
            tag: this.tagvalue.Id
          })
        )
        .then(response => {
          if (response.data.err === null) {
