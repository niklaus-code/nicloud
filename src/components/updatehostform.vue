          ...
          <div class="col-sm-8" style="float: left">
            ...
            <div style="width: 45%;float: left ">
              <div class="panel panel-success">
                ...
                <ul class="panel-body">
                  <li v-for="(item, index) in choosen_vlan" @click="backvlan(index)" :key="item.id">{{ item }}</li>
                </ul>
              </div>
            </div>
          </div>
  ...
  methods: {
    gethostbyip: function(ip) {
      ...
      this.$http.get(apiurl, { params: { ip: ip } }).then(response => {
        ...
        for (var k2 in response.data.res.unselectvlan) {
          this.vlanlist.push(response.data.res.unselectvlan[k2].Vlan)
        }
      })
    },
