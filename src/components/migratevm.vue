  ...
  <div>
    ...
    <div class="col-sm-6 col-sm-offset-3" style="margin-top:20px">
      ...
      <div class="col-sm-12 form-group" style="margin-top:20px">
        <div class="col-sm-3">
          <div class="col-sm-10 col-sm-offset-1">
            <button class="btn btn-success btn-sm" style="float:right" @click="migratevm()">
              迁移
            </button>
          </div>
        </div>
        <div class="col-sm-9">
          <select class="form-select col-sm-10" v-model="hostvalue">
            <option v-for="h in host" :value="h.Ipv4" :key="h.id"> {{ h.Ipv4 }}(cpu:{{ h.Usedcpu }}/{{ h.Cpu }}， mem:{{ h.Usedmem }}/{{ h.Mem }}， 数量:{{ h.count }}/{{ h.Max_vms }}) </option>
          </select>
        </div>
      </div>
    </div>
