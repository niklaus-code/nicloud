<template>
  <div>
    <div class="col-sm-12 form-group" style="margin-top:10px; border-bottom: 1px green solid">
      <h4>
        热迁移云主机
        <span style="color: #C0C0C0; font-size:13px">*迁移目标宿主机需开启selinux</span>
      </h4>
    </div>
    <div class="col-sm-8 col-sm-offset-2" style="margin-top:20px">
      ...
      <div class="col-sm-12">
        ...
        <div class="col-sm-9">{{ cpu }} / {{ mem }}</div>
      </div>
      <div class="col-sm-12">
        <div class="col-sm-3">
          <div class="col-sm-10 col-sm-offset-1">
      ...
      <div class="col-sm-12 form-group" style="margin-top:20px">
        <div class="col-sm-3">
          <div class="col-sm-10 col-sm-offset-1">
            <button class="btn btn-success btn-sm" style="float:right" @click="migratevmlive()">
              迁移
            </button>
          </div>
        </div>
        <div class="col-sm-9">
          <select class="form-select col-sm-10" v-model="hostvalue" style="max-width: 400px">
            <option v-for="h in host" :value="h.Ipv4" :key="h.ids"> {{ h.Ipv4 }}(cpu:{{ h.Usedcpu }}/{{ h.Cpu }}， mem:{{ h.Usedmem }}/{{ h.Mem }}， 数量:{{ h.count }}/{{ h.Max_vms }}) </option>
          </select>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    migratevmlive: function() {
      ...
      this.$http
        .get(apiurl, {
          params: { uuid: this.uuid, migratehost: this.hostvalue }
        })
        .then(response => {
