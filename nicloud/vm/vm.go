...
func VmList(userid int, start int, item string) ([]map[string]interface{}, error) {
    ...
    if role.Rolename == "admin" {
        sqlStr := fmt.Sprintf("select * from (select * from vms  order by create_time desc limit %d offset %d) v order by %s %s", offset, (start-1)*offset, item, order)
        fmt.Println(3, sqlStr)
        dbs.Raw(sqlStr).Scan(&v)
    } else {
